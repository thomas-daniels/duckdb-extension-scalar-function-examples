...
inline void ScalarFuncExamplesOpenSSLVersionScalarFun(DataChunk &args, ExpressionState &state, Vector &result) {
    auto &name_vector = args.data[0];
    UnaryExecutor::Execute<string_t, string_t>(
	    name_vector, result, args.size(),
	    [&](string_t name) {
...
static void LoadInternal(DatabaseInstance &instance) {
    ...
    auto scalar_func_examples_openssl_version_scalar_function = ScalarFunction("scalar_func_examples_openssl_version", {LogicalType::VARCHAR},
                                                LogicalType::VARCHAR, ScalarFuncExamplesOpenSSLVersionScalarFun);
    ExtensionUtil::RegisterFunction(instance, scalar_func_examples_openssl_version_scalar_function);
}
